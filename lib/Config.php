...
abstract class Config
{
    ...
    public static function getCaSeSt()
    {
        ...
        $query = Entities\Service::query( 's' )
            ->select( 's.id, s.category_id, s.title, s.position, s.duration, s.price, s.type, s.info' )
        ...
        foreach ( $query->fetchArray() as $row ) {
            $result['services'][ $row['id'] ] = array(
                ...
                'info'         => $row['info'] == '' ? '' : Utils\Common::getTranslatedString( 'service_' . $row['id'] . '_info', $row['info'] ),
                'type'         => $row['type'],
                'pos'          => (int) $row['position'],
            );
