                ...
                foreach ( $ranges->all() as $range ) {
                    ...
                    foreach ( $range->split( $this->slot_length )->all() as $slot ) {
                        ...
                        if ( $this->time_limit !== null && ! $this->srv_duration_days ) {
                            ...
                            // +24 hours if dates differs
                            $start_time += DAY_IN_SECONDS * (int) ( $slot->start()->value()->format( 'z' ) !== $this->dp->format( 'z' ) );
        ...
        if ( $ranges->isNotEmpty() ) {
            ...
            foreach ( $staff->getBookings() as $booking ) {
                ...
                if (
                    ...
                    if ( $booking->nop() - $booking->onWaitingList() <= $max_capacity - $this->nop && ! $booking->oneBookingPerSlot() ) {
